...
@interface EduHomeHeaderView()<UITableViewDelegate,UITableViewDataSource>
...
@implementation EduHomeHeaderView
-(void)setupTableView{
  ...
  self.index = 0;
    ...
    [_fiveView addGestureRecognizer:tapGesturRecognizer5];
    
    UITapGestureRecognizer *tapGesturRecognizer6 =[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapAction6:)];
    
    [_sixeView addGestureRecognizer:tapGesturRecognizer6];
    
    UITapGestureRecognizer *tapGesturRecognizer7 =[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapAction7:)];
    
    [_sevenView addGestureRecognizer:tapGesturRecognizer7];
    
    UITapGestureRecognizer *tapGesturRecognizer8 =[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapAction8:)];
    
    [_eightView addGestureRecognizer:tapGesturRecognizer8];
}

- (NSMutableArray *)tagArr {
    
    if(!_tagArr){
        _tagArr = [[NSMutableArray alloc] initWithObjects:_oneView,_twoView,_threeView,_fourView,_fiveView,_sixeView,_sevenView,_eightView, nil];
...
-(void)tapAction5:(UIView *)tap {
    self.headerTagCallBackBlcok(5);
    
    NSLog(@"4");
}
-(void)tapAction6:(UIView *)tap {
    self.headerTagCallBackBlcok(6);
    
    NSLog(@"4");
}
-(void)tapAction7:(UIView *)tap {
    self.headerTagCallBackBlcok(7);
    
    NSLog(@"4");
}
-(void)tapAction8:(UIView *)tap {
    self.headerTagCallBackBlcok(8);
