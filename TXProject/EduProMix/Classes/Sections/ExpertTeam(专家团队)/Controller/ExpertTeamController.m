...
@interface ExpertTeamController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) NSMutableArray *modelArray;
@property (nonatomic, strong) UIViewController *controller;

@property (nonatomic, strong) ExpertTeamViewModel *contentVM;
@end

@implementation ExpertTeamController


- (ExpertTeamViewModel *)contentVM {
    
    if(!_contentVM){
        _contentVM = [[ExpertTeamViewModel alloc]init];
    }
    return _contentVM;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ExpertTeamModel *model = self.contentVM.modelArr[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ExpertTeamCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ExpertTeamCell"];
    if(!cell){
        cell = [[[NSBundle mainBundle] loadNibNamed:@"ExpertTeamCell" owner:self options:nil] objectAtIndex:0];
    }
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"ExpertTeamCell" bundle:nil] forCellReuseIdentifier:@"ExpertTeamCell"];
