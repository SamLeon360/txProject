...
- (void)updateConversations
{
    ...
    NSMutableArray *convIdArr = [NSMutableArray arrayWithCapacity:0];
    for (int i = 0 ; i < _data.count; i ++) {
        TConversationCellData *cellData = _data[i];
        [convIdArr addObject:cellData.convId];
    }
    [[TIMFriendshipManager sharedInstance] getUsersProfile:convIdArr forceUpdate:YES succ:^(NSArray<TIMUserProfile *> *profiles) {
        for (int i = 0 ; i < profiles.count ; i++) {
            TConversationCellData *cellData = _data[i];
            TIMUserProfile *profile = profiles[i];
            cellData.title = profile.nickname;
            cellData.head = profile.faceURL;
        }
