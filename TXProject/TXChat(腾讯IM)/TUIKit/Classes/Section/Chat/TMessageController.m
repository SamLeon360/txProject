...
- (void)loadMessage
{
    ...
    NSInteger msgCount = config.msgCountPerRequest;
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        for (int i = 0; i < msg.elemCount; ++i) {
            ...
            if([elem isKindOfClass:[TIMTextElem class]]){
                ...
                TUIKitConfig *config = [TUIKitConfig defaultConfig];
                for (TFaceGroup *group in config.faceGroups) {
...
{
//     self.voicePlayer = [[AVAudioPlayer alloc] init];
//    __weak typeof(self) ws = self;
    __block TMessageController *blockSelf = self;
    void (^playBlock)(NSString *path) = ^(NSString *path){
        //update view
        NSInteger row = [blockSelf.uiMsgs indexOfObject:voice];
        [blockSelf.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:row inSection:0]] withRowAnimation:UITableViewRowAnimationNone];
        //play current
        if (voice.path.length <= 0) {
            voice.path = path;
        }
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        NSString *wavPath = [[voice.path stringByDeletingPathExtension] stringByAppendingString:@".wav"];
        [THelper convertAmr:voice.path toWav:wavPath];
        NSURL *url = [NSURL fileURLWithPath:wavPath];
        NSData *data = [[NSData alloc] initWithContentsOfURL:url];
        if (data == nil) {
            NSURL *url = [NSURL fileURLWithPath:voice.path];
            data = [[NSData alloc] initWithContentsOfURL:url];
        }
         NSError *error=nil;
        blockSelf.voicePlayer = [[AVAudioPlayer alloc] initWithData:data error:&error];
        [blockSelf.voicePlayer play];
...
//            [[NSFileManager defaultManager] removeItemAtPath:wavPath error:nil];
    ...
    if(voice.isPlaying){
        ...
        //update view
        NSInteger row = [self.uiMsgs indexOfObject:voice];
