...
@property (nonatomic) TChatController *chatVC;
@end
...
- (void)setupViews
{
    ...
    _textView = [[TTextView alloc] initWithFrame:CGRectMake(0, ScreenH - TTextView_Height - TTextView_Height - 20 - Bottom_SafeHeight, self.view.frame.size.width, TTextView_Height)];
...
- (void)showFaceAnimation
{
    ...
    [self.chatVCView addSubview:self.faceView];
    [self.chatVCView addSubview:self.menuView];
    ...
    frame.origin.y = self.faceView.frame.origin.y + self.menuView.frame.size.height;
    self.menuView.frame = frame;
    
    __weak typeof(self) ws = self;
    [UIView animateWithDuration:0.3 animations:^{
        CGRect newFrame = ws.faceView.frame;
        newFrame.origin.y = ws.textView.frame.origin.y - ws.textView.frame.size.height - ws.faceView.frame.size.height;
        ws.faceView.frame = newFrame;
        
        newFrame = ws.menuView.frame;
        newFrame.origin.y = ws.faceView.frame.origin.y - ws.menuView.frame.size.height;
        ws.menuView.frame = newFrame;
    } completion:^(BOOL finished) {
        NSLog(@"%lf",ws.faceView.frame.origin.y);
    }];
}
...
{
    [self.chatVCView addSubview:self.moreView];
    ...
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        newFrame.origin.y = ws.textView.frame.origin.y - ws.textView.frame.size.height - ws.moreView.frame.size.height;
...
- (void)textView:(TTextView *)textView didSendVoice:(NSString *)path
{
    ...
    voice.length = length;
    voice.uuid = USER_SINGLE.member_id;
    voice.isSelf = YES;
...
- (void)faceView:(TFaceView *)faceView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TFaceGroup *group = config.faceGroups[indexPath.section];
...
- (TMoreView *)moreView
{
    if(!_moreView){
        ...
        TUIKitConfig *config = [TUIKitConfig defaultConfig];
        [_moreView setData:config.moreMenus];
    }
    return _moreView;
}
