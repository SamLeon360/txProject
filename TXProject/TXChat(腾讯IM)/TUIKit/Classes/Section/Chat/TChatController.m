...
- (void)setupViews
{
    ...
    [leftButton setImage:[[UIImage imageNamed:TUIKitResource(@"back")] imageWithColor:[UIColor whiteColor]] forState:UIControlStateNormal];
    ...
    //input
    self.inputController = [[TInputController alloc] init];
    self.inputController.view.frame = CGRectMake(0, ScreenH - TTextView_Height - TTextView_Height - 20 - Bottom_SafeHeight, self.view.frame.size.width, TTextView_Height + Bottom_SafeHeight);
    self.inputController.delegate = self;
    self.inputController.chatVCView = self.view;
    [self addChildViewController:self.inputController];
    [self.view addSubview:self.inputController.textView];
    [self.view setBackgroundColor: TInputView_Background_Color];
}
